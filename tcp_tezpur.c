...
struct sentseg* tezpur_ins_seg(struct sentseg *segs,__u32 seq,__u32  bytes, long stime) 
       ...
       if (segs == NULL) {
		...
		return segs;
          	...
          	while (segs->next != NULL) {
       	     	     if (segs->seqno == seq) {
   			//segs->sent_time = stime;
   			//segs->sent_time = -1 ; //stime;//05/12/2004
   			segs->sent_time = 1 ; //stime;//14/01/2005
			return tmpseg;
                      }
			segs = segs->next;
		}
...
long tezpur_find_seg(struct sentseg * tmpseg,__u32 seq,__u32 * bytes) {
//__u32 tezpur_find_seg(struct sentseg * tmpseg,__u32 seq) {
	long tmp=0; //-1;
	//__u32 tmp=0;
	...
	while (tmpseg1!=NULL) {
		...
		*bytes = tmpseg1->bytes;
		//long tmp2=tezpur_now();
		//printk(KERN_ALERT "\nThe Seq. No.%u Found in the List, was sent at %u, Bytes ACKed=%u Now at %u,CRTT %u!",seq,tmp,*bytes,tmp2,tmp2-tmp);   
		return tmpseg1->sent_time;
	        //break;
	   }
...
//__u32 tezpur_find_seg_lt(struct sentseg * tmpseg,__u32 seq,__u32 * bytes) {
	long tmp=0; //-2;
	//__u32 tmp=0;
	struct sentseg *tmpseg1=tmpseg;
	...
	while (tmpseg1!=NULL) {
   	   if (tmpseg1->seqno<seq) {
	       	tmp =tmpseg1->sent_time;
		//long tmp2=tezpur_now();
		//printk(KERN_ALERT "\nThe Seq. No.%u Found in the List, was sent at %u, ACKed Now at %u,CRTT %u!",seq,tmp,tmp2,tmp2-tmp);   
...
/* Find a sequence number which is less than(lt) the given one and the biggest;11/12/2004 */
/*long tezpur_find_seg_ltmax(struct sentseg * tmpseg,__u32 seq,__u32 * bytes) {
	long tmp=-2;
	struct sentseg *tmpseg1=tmpseg;
...
void tezpur_update_rtt(struct tcp_opt *tp,__u32 seq)
{
	...
	//printk(KERN_ALERT "\nTCP Tezpur Updating RTT Now at %u for Seq# %u !",mnow,seq);

         tezpur_disp_segs(tp->tezpur.sentsegs_);
	/* Get the Current, Previous RTTs and RTT Variance */
	__u32 bytes;
	bytes=1;
        long stm  = tezpur_find_seg(tp->tezpur.sentsegs_,seq,&bytes);
	if (bytes<1)
	  bytes=1;
	   	
	//17/01/2005
        /*long stm  = 0;
	if (seq == tp->tezpur.snd_nxt_)
		stm=tp->tezpur.when_;
	*/
        //long stm  = tezpur_find_seg_lt(tp->tezpur.sentsegs_,seq,&bytes);
        //__u32 stm  = tezpur_find_seg_lt(tp->tezpur.sentsegs_,seq,&bytes);
        //__u32 stm  = tezpur_find_seg(tp->tezpur.sentsegs_,seq);
	   //printk(KERN_ALERT "\nTCP Tezpur AT Update RTT: Bytes ACKed=%u !", bytes);
        //if (stm>0) {
        if (stm>2) {
	   ...
	   tp->tezpur.sentsegs_= tezpur_del_segs_le(tp->tezpur.sentsegs_,seq);
	   //printk(KERN_ALERT "\nTCP Tezpur AT Update RTT Current RTT=%u SRTT=%u Bytes ACKed=%u Mean_RTT=%u Mean_SRTT=%u !",(__u32)tp->tezpur.crtt_,tp->srtt,bytes,(__u32)(tp->tezpur.crtt_/bytes),(__u32)(tp->srtt/bytes));
	}
        else
	  //if (stm==-1)
	  if (stm==1)
          {
	   printk(KERN_ALERT "\nTCP Tezpur Update RTT; Retransmitted Segment Found STM=1!");
           return;
         }
	  else	  
          {
	   printk(KERN_ALERT "\nTCP Tezpur Update RTT; Search Failed Exiting at STM=0!");
           return;
         }

	tezpur_threshold_rtt_estimator(tp);
	tezpur_cong_avoid(tp);
  ...
  if (tp->tezpur.WinIncrOpt_ == 3) {
   ...
   //tp->tezpur.cwndincr_ = 2;
	...
	if (tp->tezpur.rtt_thresh_ <= 0 || tp->tezpur.crtt_ <= 0)
	   ...
	   printk("\n TCP Tezpur: CRTT=%u ; Deviation(top)=%u ; Max(bottom)=%u !",(__u32)tp->tezpur.crtt_,(__u32)tmp1,(__u32)tmp2);
...
void tezpur_threshold_rtt_estimator(struct tcp_opt *tp)
{
	...
	printk(KERN_ALERT "\nTCP Tezpur;Init-Time%u: RTO= %u; CRTT = %u; Threshold RTT = %u !",tp->tezpur.init_time_,(__u32)tp->tezpur.rto_,(__u32)tp->tezpur.crtt_,(__u32)tp->tezpur.rtt_thresh_);
